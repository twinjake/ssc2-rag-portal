...
const STORAGE_KEY = "ssc2_chat_history_v1";
const FREE_LIMIT = 3;
...
export default function Home() {
  ...
  // Freemium state
  const [userStatus, setUserStatus] = useState(null); // { plan, chatCount, canChat, chatsRemaining }
  ...
  // Check for successful subscription redirect
  useEffect(() => {
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      // Refresh user status after each chat
      await fetchStatus();
    } catch (err) {
      setMessages((prev) => prev.filter((m) => m.id !== "pending"));
  ...
  const UserBubble = ({ children }) => (
    <div style={{ display: "flex", justifyContent: "flex-end", margin: "8px 0" }}>
      <div style={{ maxWidth: 760, background: "#2A2A2A", border: "1px solid #3A3A3A", color: "#EAEAEA", padding: "10px 12px", borderRadius: 12, borderTopRightRadius: 4, whiteSpace: "pre-wrap", wordBreak: "break-word", overflowWrap: "anywhere", fontFamily: "inherit" }}>
        {children}
      </div>
    </div>
  );
  ...
  const AssistantBubble = ({ children }) => (
    <div style={{ display: "flex", gap: 10, margin: "10px 0" }}>
      ...
      <div style={{ maxWidth: 760, background: "#181818", border: "1px solid #2A2A2A", color: "#EAEAEA", padding: "12px 14px", borderRadius: 12, borderTopLeftRadius: 4, wordBreak: "break-word", overflowWrap: "anywhere", fontFamily: "inherit" }}>
        {children}
      </div>
    </div>
  );
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#181818", border: "1px solid #2a2a2a", borderRadius: 20, padding: "40px 32px", maxWidth: 460, width: "100%", textAlign: "center", boxShadow: "0 8px 40px rgba(0,0,0,0.6)" }}>
        ...
        <p style={{ color: "#bdbdbd", lineHeight: 1.6, marginBottom: 28 }}>
          Subscribe for <strong style={{ color: "#0ea5e9" }}>$15.99/month</strong> to get unlimited access to Ask Dr. Spencer — AI-powered answers from Spencer Study Club content.
        </p>
        <a
          href="/api/stripe/checkout"
          style={{ display: "block", padding: "14px 20px", borderRadius: 999, background: "#0ea5e9", color: "#001018", fontWeight: 700, fontSize: 16, textDecoration: "none", marginBottom: 12 }}
        >
          Subscribe — $15.99/mo
        </a>
  ...
  // Free trial banner
  const TrialBanner = () => {
    if (!userStatus || userStatus.plan !== "free_trial" || userStatus.chatsRemaining === null) return null;
    const remaining = userStatus.chatsRemaining;
    if (remaining <= 0) return null;
    return (
      <div style={{ background: "#1e3a4a", border: "1px solid #0ea5e944", borderRadius: 8, padding: "8px 14px", marginBottom: 10, fontSize: 13, color: "#7dd3fc", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
        <span>
          {remaining === FREE_LIMIT
            ? `You have ${remaining} free chats to try Ask Dr. Spencer.`
            : remaining === 1
            ? `⚠️ This is your last free chat.`
            : `You have ${remaining} free chat${remaining !== 1 ? "s" : ""} remaining.`}
        </span>
        <a href="/pricing" style={{ color: "#0ea5e9", fontWeight: 600, textDecoration: "none", fontSize: 12 }}>Subscribe →</a>
      </div>
    );
  };
  ...
  return (
    <main style={{ background: "#212121", minHeight: "100vh", color: "#EAEAEA", display: "flex", flexDirection: "column", overflowX: "hidden" }}>
      ...
      <SignedIn>
        ...
        <section style={{ maxWidth: 900, width: "100%", margin: "0 auto", padding: "96px 16px 12px", textAlign: "center", boxSizing: "border-box" }}>
          <div style={{ display: "grid", justifyItems: "center", alignItems: "center", marginBottom: 8, rowGap: 10 }}>
            <img src="/dr-spencer.jpg" alt="Dr. Spencer" style={{ width: 100, height: 100, borderRadius: "50%", objectFit: "cover", border: "1px solid #424242" }} />
            <div style={{ textAlign: "center", maxWidth: "100%" }}>
              <div style={{ fontSize: 14, color: "#BDBDBD" }}>Ask Dr. Spencer</div>
              <h1 style={{ margin: 0, fontSize: 42, fontWeight: 800, letterSpacing: 0.2, lineHeight: 1.1, wordBreak: "break-word", fontFamily: "inherit" }}>
                SSC 2.0 – Doctor Portal
              </h1>
            </div>
          </div>

          <p style={{ margin: "8px auto 0", maxWidth: 820, color: "#D0D0D0", lineHeight: 1.6, fontSize: 18, padding: "0 4px", wordBreak: "break-word", overflowWrap: "anywhere", fontFamily: "inherit" }}>
          ...
          {messages.length === 0 && (
            <div style={{ marginTop: 20, display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 12, padding: "0 8px" }}>
              {[
                ...
                <button
                  ...
                  style={{ background: "#2A2A2A", border: "1px solid #3A3A3A", color: "#DADADA", borderRadius: 20, padding: "12px 14px", cursor: loading ? "not-allowed" : "pointer", fontSize: 14, opacity: loading ? 0.7 : 1, maxWidth: "100%", whiteSpace: "normal", textAlign: "center", boxSizing: "border-box", fontFamily: "inherit" }}
                >
                  {s}
                </button>
              ))}
        ...
        <section style={{ maxWidth: 900, width: "100%", margin: "0 auto", padding: "8px 16px 160px", boxSizing: "border-box", overflowX: "hidden" }}>
          {error && (
            <div style={{ margin: "12px auto", maxWidth: 900, color: "#FF8A80", background: "#2b1f1f", border: "1px solid #4a2a2a", padding: 12, borderRadius: 10, fontFamily: "inherit" }}>
              <b>Error:</b> {error}
            </div>
          )}
        ...
        <form
          onSubmit={onSubmit}
          style={{ position: "fixed", left: 0, right: 0, bottom: 0, padding: "16px 16px calc(env(safe-area-inset-bottom, 0px) + 24px)", background: "linear-gradient(180deg, rgba(33,33,33,0) 0%, rgba(33,33,33,0.85) 30%, #212121 65%)", display: "flex", flexDirection: "column", alignItems: "center", zIndex: 9, overflowX: "hidden" }}
        >
          {/* Trial banner */}
          ...
          {/* Topic selector */}
          <div style={{ width: "100%", maxWidth: 900, margin: "0 auto 10px", display: "grid", gap: 8, justifyItems: "center", boxSizing: "border-box" }}>
            <div role="radiogroup" aria-label="Question topic" style={{ display: "flex", background: "#2A2A2A", border: "1px solid #3A3A3A", borderRadius: 9999, padding: 4, gap: 4 }}>
              {["tmd", "sleep", "both"].map((key) => {
                ...
                return (
                  <button key={key} type="button" role="radio" aria-checked={selected} onClick={() => setTopic(key)}
                    style={{ cursor: "pointer", border: "none", outline: "none", padding: "8px 14px", borderRadius: 9999, background: selected ? "#1976D2" : "transparent", color: selected ? "#FFFFFF" : "#E0E0E0", fontSize: 14, fontFamily: "inherit", transition: "background 0.2s, color 0.2s" }}>
                    {label}
                  </button>
                );
              })}
          ...
          {/* Input bar */}
          <div style={{ maxWidth: 900, width: "100%", display: "grid", gridTemplateColumns: "48px 1fr 48px", alignItems: "center", gap: 8, background: "#2A2A2A", border: "1px solid #3A3A3A", borderRadius: 28, padding: "6px 8px", boxSizing: "border-box", fontFamily: "inherit" }}>
            <button type="button" onClick={toggleMic} title="Voice input"
              style={{ width: 40, height: 40, borderRadius: 20, border: "none", background: listening ? "#3949AB" : "transparent", cursor: "pointer", display: "grid", placeItems: "center", color: listening ? "#EAEAEA" : "#BDBDBD", fontFamily: "inherit" }}>
            ...
            <textarea
              ...
              style={{ resize: "none", background: "transparent", border: "none", outline: "none", color: "#EAEAEA", padding: "10px 6px", fontSize: 16, lineHeight: 1.4, maxHeight: 140, overflowX: "hidden", fontFamily: "inherit", opacity: userStatus && !userStatus.canChat ? 0.5 : 1 }}
            ...
            <button type="submit" disabled={loading || !q.trim() || (userStatus && !userStatus.canChat)} title="Send"
              style={{ width: 40, height: 40, borderRadius: 20, border: "none", background: loading || !q.trim() || (userStatus && !userStatus.canChat) ? "#37474F" : "#1976D2", cursor: loading || !q.trim() || (userStatus && !userStatus.canChat) ? "not-allowed" : "pointer", display: "grid", placeItems: "center", color: "#fff", transition: "background 0.2s", fontFamily: "inherit" }}>
